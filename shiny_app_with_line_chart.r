...
server <- function(input, output, session) {
  ...
  output$bar_info <- renderUI({
    ...
    if (length(potential_groups) == 0) {
      div(
        style = "background-color: #fff3cd; padding: 10px; border-radius: 5px; border: 1px solid #ffeaa7;",
        HTML("<strong>Note:</strong> No categorical variable detected. Bar chart needs a third column (category, region, department, etc.) to create bars.")
      )
    } else {
      group_var <- potential_groups[1]
      unique_groups <- unique(data[[group_var]])
  ...
  run_plot <- reactive({
    ...
    ggplot(data, aes(x = date, y = value)) +
      ...
      # Y-axis formatting: percentages, 10% expansion
      scale_y_continuous(
      ...
      theme(
        ...
        plot.title = element_markdown(color = "darkgreen", size = 33, face = "bold", lineheight = 1.1, margin = margin(2, 0, 0, 0, "lines")),
        plot.subtitle = element_markdown(color = "darkgreen", size = 28, face = "bold", lineheight = 1.0, margin = margin(0, 0, 0, 0, "lines")),
        plot.caption = element_text(size = 25, hjust = 0, vjust = 2, face = "italic", color = "darkblue"),
  ...
  line_plot <- reactive({
    ...
    ggplot(data, aes(x = date, y = value, color = !!sym(group_var))) +
      ...
      geom_text(data = label_data,
                ...
                hjust = -0.1, vjust = 0.5, size = 8, fontface = "bold") +

# Ultimate EDA Shiny App - Complete Version with Fixed Recalculation
# Clean, working version with Run Chart, Line Chart, and Bar Chart
# FIXED: Recalculation annotation positioning and reactivity
                ...
                div(style = "background-color: #f8f9fa; padding: 10px; border-radius: 5px; border: 1px solid #dee2e6;",
                  fluidRow(
                    ...
                    column(5,
                      conditionalPanel(
                        condition = "input.enable_recalc",
                        tags$small("Select date to split process and recalculate control limits", 
...
server <- function(input, output, session) {
  ...
  # FIXED: Create reactive plot for control chart with improved recalculation
  control_plot <- reactive({
    ...
    if(recalc_enabled) {
      # RECALCULATION MODE: Split data and calculate separate control limits
      ...
      # Calculate original control chart statistics (from before segment or full data)
      ...
      # Calculate recalculated control chart statistics (from after segment)
      ...
      # Calculate runs signals across both segments
      data$above_cl <- ifelse(data$date < recalc_date,
                             data$value > emp_cl_orig,
                             data$value > emp_cl_recalc)
      data$runs_signal <- FALSE
      
      # Simple runs test implementation across segments
      ...
      # STANDARD MODE: Original untrended control chart
    ...
    if(recalc_enabled) {
      # RECALCULATION MODE: Show both sets of control limits
      ...
      # Original control limits (before recalc date)
      ...
      # Recalculated control limits (after recalc date)
      ...
      # STANDARD MODE: Original control chart display
      p <- p + 
